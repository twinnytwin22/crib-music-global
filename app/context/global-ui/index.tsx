...
export const GlobalUIProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
      ...
      <GlobalUIContext.Provider value={values}>
        <div className={`${showModal && 'pr-2 w-screen h-screen overflow-clip '}`}>
          {children}
          {showModal && (
            <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center z-50 bg-opacity-50 bg-black mr-2  ">
              {song &&
              <div className={`fixed md:ml-10 max-w-screen mx-auto z-20 min-h-screen justify-center left-0 w-full p-4 top-1/3 rounded-md place-items-center mr-2 `}>
                {/* Modal */}
